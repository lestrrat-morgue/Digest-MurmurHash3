...
STATIC_INLINE uint32_t ROTL32( uint32_t x, int8_t r ) {
    return ( x << r ) | ( x >> ( 32 - r ) );
}

#ifdef HAVE_64BITINT
STATIC_INLINE uint64_t ROTL64( uint64_t x, int8_t r ) {
    return ( x << r ) | ( x >> ( 64 - r ) );
}
#endif
...
STATIC_INLINE uint32_t getblock32 ( const uint32_t * p, int i ) {
  return p[i];
}

#ifdef HAVE_64BITINT
STATIC_INLINE uint64_t getblock64 ( const uint64_t * p, int i ) {
  return p[i];
}
#endif

STATIC_INLINE uint32_t fmix32 ( uint32_t h ) {
    h ^= h >> 16;
    h *= 0x85ebca6b;
    h ^= h >> 13;
    h *= 0xc2b2ae35;
...
#ifdef HAVE_64BITINT
STATIC_INLINE uint64_t fmix64 ( uint64_t k ) {
  k ^= k >> 33;
...
/* Taken from MurmurHash3.cpp 5/18/2011 */
STATIC_INLINE
